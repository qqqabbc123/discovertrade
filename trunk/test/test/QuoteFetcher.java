...
public class QuoteFetcher {
	...
	static void run() {
		...
		while (!isMartketOpen()) {
			try {
				System.out.println("Market not open will wait for " + waitTime
						/ 1000 + " seconds");
		...
		try {
			String[] headers = new String[] { "Date", "Open", "High", "Low",
					"Close", "Volume" };
		...
		// previous quote.
		TextQuoteReader quoteReader = new TextQuoteReader(new Security(), null,
				iciciDirectTemplate);
				...
				try {
					...
					if (currQuote != null) {
						...
						if (prevQuote == null) {
							...
							quote
									.setHigh(prevQuote.getLastTradedPrice() > currQuote
											.getLastTradedPrice() ? prevQuote
											.getLastTradedPrice() : currQuote
											.getLastTradedPrice());
							quote
									.setLow(prevQuote.getLastTradedPrice() < currQuote
											.getLastTradedPrice() ? prevQuote
											.getLastTradedPrice() : currQuote
											.getLastTradedPrice());
							quote.setVolume(currQuote.getVolume()
									- prevQuote.getVolume());
